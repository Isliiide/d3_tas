...
int skip_damage_to_agree_with_slomo = 0; //islide
...
void DoFusionEffect(object *objp, int weapon_type) {
  ...
  if (norm > 1.0) {
    ...
    if ((!(Game_mode & GM_MULTI)) || Netgame.local_role == LR_SERVER) {
      ...
      //----------------------
      int slomo = get_slomo_factor();
      int zero_damage = 0;
      if (skip_damage_to_agree_with_slomo == 0) {//islide

        zero_damage = 1;
      }
      skip_damage_to_agree_with_slomo++;
      skip_damage_to_agree_with_slomo = skip_damage_to_agree_with_slomo % slomo;
      //-----------------


      if ((Gametime - Last_fusion_damage_time) < (FUSION_DAMAGE_SOUND_INTERVAL)) {
        ApplyDamageToPlayer(objp, objp, PD_ENERGY_WEAPON, (.125 * abs(move) * zero_damage), 0, 255, 0); // last 0 is play_sound = false
      } else {
          ApplyDamageToPlayer(objp, objp, PD_ENERGY_WEAPON, (.125 * abs(move) * zero_damage));
